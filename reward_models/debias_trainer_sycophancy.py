...
def post_process_HS3(rewards_j: np.ndarray, rewards_k: np.ndarray, metric_key_prefix: str, bias_labels: np.ndarray) -> Dict[str, float]:
    """
    后处理函数，用于计算 HelpSteer3-like 数据集的评测指标。
    支持根据 bias_labels (0 for natural, 1 for adversarial) 进行分组计算。
    """

    total_acc = (rewards_j > rewards_k).astype(np.float32).mean()
    total_margin = (rewards_j - rewards_k).astype(np.float32).mean()
    
    metrics = {
        f"{metric_key_prefix}_HS3_accuracy_overall": total_acc.item(),
        f"{metric_key_prefix}_HS3_avg_margin_overall": total_margin.item(),
        f"{metric_key_prefix}_HS3_avg_rewards_chosen_overall": rewards_j.mean().item(),
        f"{metric_key_prefix}_HS3_avg_rewards_rejected_overall": rewards_k.mean().item(),
    }
    
    natural_mask = (bias_labels == 0)
    if np.any(natural_mask):
        natural_rewards_j = rewards_j[natural_mask]
        natural_rewards_k = rewards_k[natural_mask]
        
        natural_acc = (natural_rewards_j > natural_rewards_k).astype(np.float32).mean()
        natural_margin = (natural_rewards_j - natural_rewards_k).astype(np.float32).mean()
        
        metrics.update({
            f"{metric_key_prefix}_HS3_natural_accuracy": natural_acc.item(),
            f"{metric_key_prefix}_HS3_natural_avg_margin": natural_margin.item(),
        })
    else:
        metrics.update({
            f"{metric_key_prefix}_HS3_natural_accuracy": np.nan,
            f"{metric_key_prefix}_HS3_natural_avg_margin": np.nan,
        })

    adversarial_mask = (bias_labels == 1)
    
    if np.any(adversarial_mask):
        adversarial_rewards_j = rewards_j[adversarial_mask]
        adversarial_rewards_k = rewards_k[adversarial_mask]

        adversarial_acc = (adversarial_rewards_j > adversarial_rewards_k).astype(np.float32).mean()
        adversarial_margin = (adversarial_rewards_j - adversarial_rewards_k).astype(np.float32).mean()
        
        metrics.update({
            f"{metric_key_prefix}_HS3_adversarial_accuracy": adversarial_acc.item(),
            f"{metric_key_prefix}_HS3_adversarial_avg_margin": adversarial_margin.item(),
        })
    else:
        metrics.update({
            f"{metric_key_prefix}_HS3_adversarial_accuracy": np.nan,
            f"{metric_key_prefix}_HS3_adversarial_avg_margin": np.nan,
        })
...
class DeBiasedRewardTrainer(RewardTrainer): 
    ...
    def evaluation_loop(
        ...
        if self.accelerator.is_main_process:
            ...
            if _dataset_name == 'RBv1':
                ...
                metrics = post_process_RBv1(rewards_j, rewards_k, metric_key_prefix)
            elif _dataset_name == 'HS3':
                bias_labels = self.eval_dataset['bias_label'].cpu().numpy()
                metrics = post_process_HS3(rewards_j, rewards_k, metric_key_prefix, bias_labels)
            else:
                acc = (rewards_j > rewards_k).astype(np.float16).mean()
                reward_diff = (rewards_j - rewards_k).astype(np.float16).mean()
                metrics = {
                    f"{metric_key_prefix}_Eval_accuracy": acc.item(),
                    f"{metric_key_prefix}_Eval_avg_rewards_chosen": rewards_j.mean().item(),
                    f"{metric_key_prefix}_Eval_avg_rewards_rejected": rewards_k.mean().item(),
                    f"{metric_key_prefix}_Eval_avg_margin": reward_diff.item(),
                }
        else:
            metrics = {}
    ...
    #         loss = self.compute_loss(model, inputs, num_items_in_batch=num_items_in_batch)
        ...
        with self.compute_loss_context_manager():
            ...
            A_labels = inputs["bias_label"]
            A_labels = torch.tensor(A_labels).long().to(self.accelerator.device)
